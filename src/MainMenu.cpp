...
void MainMenu::Init()
{
    m_context->m_assets->AddFont(MAIN_FONT, CONCERT_ONE);

    // Title
    m_gameTitle.setFont(m_context->m_assets->GetFont(MAIN_FONT));
    m_gameTitle.setString(GAME_TITLE);
    m_gameTitle.setOrigin(m_gameTitle.getLocalBounds().width / 2.f,
                          m_gameTitle.getLocalBounds().height / 2.f);
    m_gameTitle.setPosition(m_context->m_window->getSize().x / 2.f,
                            m_context->m_window->getSize().y / 2.f - 150.f);

    // Play Button
    m_playButton.setFont(m_context->m_assets->GetFont(MAIN_FONT));
    m_playButton.setString(PLAY_BUTTON_TITLE);
    m_playButton.setOrigin(m_playButton.getLocalBounds().width / 2.f,
                           m_playButton.getLocalBounds().height / 2.f);
    m_playButton.setPosition(m_context->m_window->getSize().x / 2.f,
